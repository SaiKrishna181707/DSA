...
lists or arrays. For example, schedule[0][0].start = 1, schedule[0][0].end = 2, and
schedule[0][0][0] is not defined).  Also, we wouldn't include intervals like [5, 5] in our answer, as
...
public class LC_759 {
    public static void main(String[] args) {
        ...
        Arrays.sort(employee,(a,b) -> Integer.compare(a[0], b[0]));

        List<int[]> list = new ArrayList<>();
        int end = employee[0][1];

        for(int i = 1; i < n; i++){

            int start = employee[i][0];

            if(start > end){
                list.add(new int[]{end, start});
            }
            end = Math.max(end, employee[i][1]);
        }
        for(int[] free : list){
            System.out.println(free[0] + " " + free[1]);
        }
        sc.close();
    }
}
